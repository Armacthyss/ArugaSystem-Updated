<template>
  <div class="w-full min-h-screen bg-[#FDFCF7] flex justify-center font-sans antialiased text-slate-900">
    <div class="w-full max-w-[1250px] px-6 py-6">
      ...
      <header class="relative z-[100] flex justify-between items-center mb-6">
        ...
        <div class="flex items-center gap-4">
          <button @click="showNotifications = true" class="relative w-10 h-10 rounded-full bg-white border border-slate-200 flex items-center justify-center shadow-sm hover:bg-slate-50 transition-colors">
            ...
            <span v-if="unreadCount > 0" class="absolute -top-1 -right-1 bg-[#ff4d4d] text-white text-[8px] font-bold px-1.5 py-0.5 rounded-full min-w-[18px] text-center">
      ...
      <!-- 2. MAIN NAVIGATION -->
      <nav class="flex items-center justify-between bg-white border border-slate-200/60 p-2 rounded-[24px] shadow-sm mb-10 sticky top-4 z-50 backdrop-blur-md gap-3">
      ...
      <div class="grid grid-cols-12 gap-8">
        ...
        <aside class="col-span-12 lg:col-span-3 space-y-4">
          ...
          <div class="space-y-2 max-h-[calc(100vh-220px)] overflow-y-auto pr-1">
            <button v-for="child in children" :key="child.childID"
              ...
              <span class="text-xl flex-shrink-0">{{ child.sex === 'Female' ? '👧' : '👶' }}</span>
              <div class="min-w-0">
        ...
        <main class="col-span-12 lg:col-span-9">
          ...
          <div v-if="activeNav === 'Overview'" class="space-y-5 animate-in slide-in-from-bottom-4 duration-500">
            <div class="bg-white rounded-[32px] border border-slate-100 shadow-sm p-8 flex flex-col md:flex-row justify-between items-center gap-6">
              <div>
              ...
              <div class="bg-[#f0f2ed] px-10 py-6 rounded-[28px] border border-[#99ad7a]/20 text-center min-w-[180px]">
            ...
            <div v-if="selectedChild" class="bg-white rounded-[32px] border border-slate-100 shadow-sm overflow-hidden">
          ...
          <div v-else-if="activeNav === 'Check-in'" class="animate-in fade-in zoom-in-95 duration-500 space-y-4">
            <div v-if="selectedChild" class="bg-white rounded-[28px] border border-slate-100 shadow-sm p-5 flex items-center gap-5">
              <div class="w-12 h-12 rounded-2xl bg-[#f0f2ed] flex items-center justify-center text-2xl flex-shrink-0">{{ selectedChild.sex === 'Female' ? '👧' : '👶' }}</div>
              ...
              <div v-if="upcomingDoses.length > 0" class="text-right hidden sm:block flex-shrink-0">
            ...
            <div class="bg-[#2d3a26] rounded-[32px] p-10 text-center text-white relative overflow-hidden shadow-2xl">
              ...
              <div class="relative">
                ...
                <div class="relative w-56 h-56 mx-auto mb-8">
                  ...
                  <div class="absolute inset-x-0 top-0 h-0.5 bg-gradient-to-r from-transparent via-[#99ad7a] to-transparent shadow-[0_0_12px_#99ad7a]" style="animation: scanline 2s ease-in-out infinite;"></div>
          ...
          <div v-else-if="activeNav === 'Schedule'" class="animate-in fade-in slide-in-from-right-4 duration-500">
            ...
            <div v-else class="flex gap-5 items-start">
              <div class="w-[42%] flex-shrink-0 flex flex-col bg-white rounded-[32px] border border-slate-100 shadow-sm overflow-hidden" style="max-height: 680px;">
                ...
                <div class="overflow-y-auto flex-1 p-4 space-y-2">
                  <template v-for="(group, gIdx) in groupedVaccineList" :key="gIdx">
                    ...
                    <button v-for="vax in group.doses" :key="vax.doseId" @click="selectedVax = vax"
                      ...
                      <div class="flex items-center gap-3">
                        <span :class="selectedVax?.doseId === vax.doseId ? 'bg-white/20' : vax.isCompleted ? 'bg-green-100' : 'bg-white'" class="w-7 h-7 rounded-lg flex items-center justify-center text-xs shadow-sm flex-shrink-0">
                          {{ vax.isCompleted ? '✅' : '💉' }}
                        </span>
                        <div>
                      ...
                      <span v-if="selectedVax?.doseId === vax.doseId" class="text-[9px] font-black text-white/80 uppercase tracking-wide flex-shrink-0">Viewing →</span>
                      <span v-else-if="vax.isCompleted" class="text-[9px] font-black text-green-600 uppercase flex-shrink-0">✓ Done</span>
                    </button>
                  </template>
                </div>
              </div>
              <div class="flex-1 flex flex-col gap-4 sticky top-[88px]">
          ...
          <div v-else-if="activeNav === 'Records'" class="space-y-5 animate-in fade-in slide-in-from-right-4 duration-500">
            <div class="grid grid-cols-3 gap-4">
              <div class="bg-[#5d6b52] p-6 rounded-[24px] text-center text-white shadow-sm"><h3 class="text-4xl font-black">{{ recordStats.completed }}</h3><p class="text-[9px] uppercase font-black opacity-70 mt-1 tracking-widest">Completed</p></div>
              <div class="bg-[#99ad7a] p-6 rounded-[24px] text-center text-white shadow-sm"><h3 class="text-4xl font-black">{{ recordStats.scheduled }}</h3><p class="text-[9px] uppercase font-black opacity-70 mt-1 tracking-widest">Scheduled</p></div>
              <div class="bg-[#e57373] p-6 rounded-[24px] text-center text-white shadow-sm"><h3 class="text-4xl font-black">{{ recordStats.overdue }}</h3><p class="text-[9px] uppercase font-black opacity-70 mt-1 tracking-widest">Overdue</p></div>
            </div>
      ...
      <Transition name="modal-fade">
        <div v-if="showProfile" class="fixed inset-0 z-[300] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" @click.self="showProfile = false">
          <div class="relative w-full max-w-2xl bg-white rounded-[32px] shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
            <div class="bg-[#5d6b52] px-8 py-7 flex items-center gap-5 flex-shrink-0">
              <div class="w-16 h-16 rounded-2xl bg-white/20 flex items-center justify-center text-4xl shadow-lg flex-shrink-0">👤</div>
              <div class="flex-1 min-w-0">
              ...
              <button @click="showProfile = false" class="w-9 h-9 rounded-xl bg-white/10 hover:bg-white/25 flex items-center justify-center text-white text-lg transition-all flex-shrink-0">✕</button>
            </div>
  ...
  <div v-if="showChangePw" class="mt-3 space-y-3 px-1">
    ...
    <p class="text-[9px] text-slate-400 text-center font-bold px-2">
      After changing your password, you'll need to log in again next time.
                ...
                <div v-else class="space-y-3">
                  <div v-for="child in children" :key="child.childID" class="border border-slate-100 rounded-2xl overflow-hidden">
                    <button @click="toggleChildCard(child.childID)" class="w-full flex items-center gap-4 px-5 py-4 bg-slate-50 hover:bg-slate-100 transition-colors text-left">
                      <div class="w-10 h-10 rounded-xl bg-[#5d6b52]/10 flex items-center justify-center text-xl flex-shrink-0">{{ child.sex === 'Female' ? '👧' : '👶' }}</div>
                      <div class="flex-1 min-w-0">
            ...
            <div class="flex-shrink-0 px-8 py-5 border-t border-slate-100 bg-[#f8f9f5] flex items-center justify-between gap-4">
              <p class="text-[10px] text-slate-400 font-bold">To update your information, please contact the clinic staff.</p>
              <button @click="showProfile = false" class="bg-[#5d6b52] hover:bg-[#4a5741] text-white px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-colors flex-shrink-0">Close</button>
            </div>
          </div>
        </div>
      </Transition>
      ...
      <Transition name="notif-panel">
        <div v-if="showNotifications" class="fixed inset-0 z-[200] flex justify-end" @click.self="showNotifications = false">
          ...
          <div class="relative w-full max-w-[400px] h-full bg-white shadow-2xl flex flex-col">
            <div class="bg-[#5d6b52] px-6 py-5 flex items-center justify-between text-white flex-shrink-0">
              <div class="flex items-center gap-3">
            ...
            <div class="flex gap-1 p-3 bg-[#f8f9f5] border-b border-slate-100 flex-shrink-0">
              ...
              <div v-else class="p-3 space-y-2">
                <div v-for="notif in filteredNotifications" :key="notif.notificationID" @click="markRead(notif)"
                  ...
                  <div class="flex items-start gap-3">
                    <div :class="notifIconBg(notif.type)" class="w-10 h-10 rounded-xl flex items-center justify-center text-lg flex-shrink-0 shadow-sm">{{ notifIcon(notif.type) }}</div>
                    <div class="flex-1 min-w-0">
                      <div class="flex items-start justify-between gap-2">
                        <p class="text-xs font-black text-slate-800 leading-tight">{{ notif.title }}</p>
                        <span v-if="!notif.isRead" class="w-2 h-2 rounded-full bg-[#99ad7a] flex-shrink-0 mt-1"></span>
                      </div>
            ...
            <div class="flex-shrink-0 px-6 py-4 border-t border-slate-100 bg-[#f8f9f5]">
...
onMounted(async () => {
  ...
  try {
    const res = await axios.get(`${API}/api/Children/parent/${parentData.value.parentID}`)
