...
async function loadSessions() {
  loading.value = true
  try {
    // Try the all-completed endpoint first
    const res = await axios.get(`${API}/api/VaccinationRecords/all-completed`)
    completedSessions.value = res.data.map(normalizeRecord)
  } catch {
...
function normalizeRecord(r) {
  return {
    ...
    dateAdministered:  r.dateAdministered ? new Date(r.dateAdministered) : null,
