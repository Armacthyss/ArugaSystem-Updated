      ...
      <main class="flex-1 overflow-y-auto p-6">
        ...
        <div class="grid grid-cols-3 gap-6">
          ...
          <div class="flex flex-col gap-4">
            ...
            <!-- VACCINATED TODAY -->
            <!--
              Sourced from GET /api/VaccinationRecords, filtered client-side to
...
onMounted(() => {
  ...
  const u = stored
    ? JSON.parse(stored)
    : { UserID: 'dev-test-user', FirstName: 'Test', LastName: 'Worker', UserType: 'Nurse' }
...
// GET /api/VaccinationRecords — filtered client-side by
...
async function fetchRecentlyVaccinated() {
  ...
  try {
    const res = await fetch(`${API_BASE}/VaccinationRecords`, { headers: authHeaders() })
    ...
    recentlyVaccinated.value = data
      ...
      .map(r => ({
        ...
        childName: r.child
          ? `${r.child.firstName ?? r.child.FirstName ?? ''} ${r.child.lastName ?? r.child.LastName ?? ''}`.trim()
          : (r.Child ? `${r.Child.FirstName ?? ''} ${r.Child.LastName ?? ''}`.trim() : 'Unknown'),
        vaccineName: r.vaccine?.vaccineName ?? r.Vaccine?.VaccineName ?? 'Unknown',
